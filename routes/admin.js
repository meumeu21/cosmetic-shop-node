...
router.post('/products/add', ensureAuthenticated, async (req, res) => {
    try {
        const { name, description, price, stock_quantity, category, image_url,
            brand, age_group, volume, items_in_set, is_hypoallergenic, application,
            composition, contraindications, type
         } = req.body;
...
router.post('/customers/add', ensureAdmin, async (req, res) => {
    try {
        ...
        await Customer.create({ first_name, last_name, email, password, phone, address });

document.addEventListener('DOMContentLoaded', function () {
  ...
  document.querySelectorAll('#categoryOpenFilter button').forEach((btn, index) => {
  ...
  function filterAndSortProducts() {
    ...
    const productsData = allProducts.map(productEl => {
      return {
        element: productEl,
        category: productEl.querySelector('div > span:nth-child(1)').textContent.trim(),
        price: parseFloat(productEl.querySelector('div > span:nth-child(2)').textContent.replace(/[^\d.]/g, ''))
      };
    });

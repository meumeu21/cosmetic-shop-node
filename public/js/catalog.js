document.addEventListener('DOMContentLoaded', function () {
  ...
  allOriginalProducts.forEach(product => productsStorage.appendChild(product.cloneNode(true)));
  ...
  function generatePageButtons() {
    ...
    if (totalPages <= 5) {
      for (let i = 2; i <= totalPages; i++) buttons.push(createPageNumberButton(i));
    } else {
      if (currentPage <= 3) {
        buttons.push(createPageNumberButton(2), createPageNumberButton(3), createEllipsis(), createPageNumberButton(totalPages - 1), createPageNumberButton(totalPages));
      } else if (currentPage >= totalPages - 2) {
        buttons.push(createPageNumberButton(2), createEllipsis());
        for (let i = totalPages - 2; i <= totalPages; i++) buttons.push(createPageNumberButton(i));
      } else {
        buttons.push(createEllipsis(), createPageNumberButton(currentPage - 1), createPageNumberButton(currentPage), createPageNumberButton(currentPage + 1), createEllipsis(), createPageNumberButton(totalPages));
      }
    }
  ...
  function initPagination() {
    ...
    container.className = 'pagination flex justify-center items-center gap-2 mt-[80px]';

    container.appendChild(createPaginationButton('first', '/svgs/arrow-start-pag.svg'));
    container.appendChild(createPaginationButton('prev', '/svgs/arrow-left-pag.svg'));

    const pageContainer = document.createElement('div');
    pageContainer.className = 'flex items-center gap-2';
    container.appendChild(pageContainer);

    container.appendChild(createPaginationButton('next', '/svgs/arrow-right-pag.svg'));
    container.appendChild(createPaginationButton('last', '/svgs/arrow-end-pag.svg'));

    productListContainer.parentNode.insertBefore(container, productListContainer.nextSibling);
  }

  function attachFilterEvents() {
    categoryFilterBtn.addEventListener('click', e => toggleFilter(e, categoryOpenFilter, categoryFilterBtn));
    sortFilterBtn.addEventListener('click', e => toggleFilter(e, sortOpenFilter, sortFilterBtn));
    ...
    sortOpenFilter.querySelectorAll('button').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        currentSort = label.includes('возрастанию') ? 'asc' : label.includes('убыванию') ? 'desc' : 'default';
        filterAndSortProducts();
        closeAllFilters();
      });
    });
  ...
  function filterAndSortProducts() {
    const all = Array.from(productsStorage.children).map(el => ({
      ...
      price: parseFloat(el.querySelector('div > span:nth-child(2)').textContent.replace(/[^\d.]/g, ''))
    }));

    let result = all;
    if (currentCategory !== 'Все') result = result.filter(p => p.category === currentCategory);
